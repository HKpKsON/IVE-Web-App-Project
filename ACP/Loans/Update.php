...
if($userRepo->find($_SESSION['uid'])->isAdmin == 255){
	...
	$Loan = new Loans;
	$Loan = isset($_GET['loan']) ? $LoansRepo->find($_GET['loan']) : FALSE;
	
	if(isset($_POST['url']) && $Loan !== FALSE){
		...
		if($uploaded){
			try{
				unlink($_SERVER['DOCUMENT_ROOT'] ."/Uploads/".$Loan->logo);
			}catch(Exception $e){
		...
		$Loan->url = $_POST['url'] == '' ? $Loan->url : $_POST['url'];
		$Loan->content = $_POST['content'] == '' ? $Loan->content : $_POST['content'];
		
		$stat = $LoansRepo->update($Loan);
